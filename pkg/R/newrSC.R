...
newrSC <- function(n = 1, MT = 20, B.start = 6, design = NULL, m = 50, s = 10, prob = 0.5, d.trend = 0, d.level = 0.0, d.slope = 0.0, rtt = 0.80, round = NA, extreme.p = 0, extreme.d = c(-4,-3), missing.p = 0, distribution = "normal", start.values.fixed = FALSE, random.names = FALSE, output.long = FALSE) {
  ...
  if(is.null(design)) {
    ...
    for(i in 1:(n*cases)) {
      design[[i]]$mt    <- sum(design[[i]]$length)
      design[[i]]$rtt     <- rtt[i]
      design[[i]]$error     <- error[i]
      design[[i]]$missing.p  <- missing.p[i]
      design[[i]]$extreme.p  <- extreme.p[i]
      design[[i]]$extreme.low  <- extreme.d[1]
      design[[i]]$extreme.high  <- extreme.d[2]
      design[[i]]$trend <- d.trend[i]
      design[[i]]$level <- c(0,d.level[i])
      design[[i]]$slope <- c(0,d.slope[i])
      design[[i]]$m     <- m[i]
      design[[i]]$s     <- s[i]
      
      design[[i]]$start <- c(1,cumsum(design[[i]]$length)+1)[1:length(design[[i]]$length)]
      design[[i]]$stop <- cumsum(design[[i]]$length)
  ...
  for (i in 1:n) {
    if(distribution == "normal") {
      start_values <- rep(design[[i]]$m[1], design[[i]]$mt[1])
      if(is.null(design) && !start.values.fixed)
        start_values <- rep(rnorm(1, design[[i]]$m[1], design[[i]]$s[1]), design[[i]]$mt[1])	

      trend_values <- (0:(design[[i]]$mt[1]-1))*design[[i]]$trend[1]*design[[i]]$s[1]
      slope_values <- c()
      level_values <- c()
      for(i2 in 1:nrow(design[[i]])) {
        slope_values <- c(slope_values, (1:design[[i]]$length[i2])*design[[i]]$slope[i2]*design[[i]]$s[i2])
        level_values <- c(level_values, rep(design[[i]]$level[i2]*design[[i]]$s[i2], design[[i]]$length[i2]))
      }
      true_values     <- start_values + trend_values + slope_values + level_values
      error_values    <- rnorm(design[[i]]$mt[1], mean = 0, sd = design[[i]]$error[1])
    ...
    if(design[[i]]$extreme.p[1] > 0){
      ra <- runif(design[[i]]$mt[1])
      if(distribution == "normal")
        multiplier <- design[[i]]$s[1]
      if (distribution == "binomial" || distribution == "poisson")
        multiplier <- 1
      for(k in 1:design[[i]]$mt[1]) {
        if(ra[k] <= design[[i]]$extreme.p[1])
          measured_values[k] <- measured_values[k] + (runif(1, design[[i]]$extreme.low[1], design[[i]]$extreme.high[1]) * multiplier) 
      }
    }
    
    if(design[[i]]$missing.p[1] > 0){
      measured_values[sample(1:design[[i]]$mt[1], design[[i]]$missing.p[1]*design[[i]]$mt[1])] <- NA
    }
    ...
    condition <- rep(design[[i]]$phase,design[[i]]$length)
    
    
    dat[[i]] <- data.frame(phase = condition, values = measured_values, mt = 1:design[[i]]$mt[1])
    if (output.long){
