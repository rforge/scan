...
newtauUSC <- function (data, ties.method = "omit", method = "parker", phases = c("A","B")) {
  ...
  for(i in 1:N) {
    out <- matrix(rep(NA,8*12),8,12, dimnames= list(c("A vs. B","Trend A","Trend B","Trend B - Trend A","Full","A vs. B - Trend A","A vs. B + Trend B", "A vs. B + Trend B - Trend A"),c("pairs", "pos", "neg","ties","S","D","Tau","Tau.b","SD","VAR","Z","p")))
    
    out <- as.data.frame(out)
    
    A <- data[[i]][data[[i]][,1] == "A",2]
    B <- data[[i]][data[[i]][,1] == "B",2]  
    AB <- c(A,B)
    nA <- length(A)
    nB <- length(B)
    nAB <- nA+nB
    ...
    out$VAR <- out$SD^2
    out$Z <- out$S/out$SD
    out$p <- pnorm(abs(out$Z), lower = FALSE)*2
