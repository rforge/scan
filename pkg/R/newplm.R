...
newplm <- function(data, AR = 0, model = "B&L-B", phases = NULL, family = "gaussian", formula = NULL, na.action = na.omit, ...) {
  ...
  ### model definition
  dat_inter <- .plm.predictor(data, model = "B&L-B")#, model = model)
...
.plm.predictor <- function(data, model, dummy = TRUE) {
  ...
  if(model == "B&L-B") {
    for(phase in 2:length(design$values)) {
      ...
      length.phase <- design$lengths[phase]
      pre <- sum(design$lengths[1:(phase-1)])
      inter[(pre +1):(pre + length.phase)] <- 1:length.phase
  ...
  if(model == "H&M") {
      for(phase in 2:length(design$values)) {
        ...
        length.phase <- design$lengths[phase]
        pre <- sum(design$lengths[1:(phase-1)])
        inter[(pre +1):(pre + length.phase)] <- 0:(length.phase-1)
        out[,paste0("inter",design$values[phase])] <- inter
    }
