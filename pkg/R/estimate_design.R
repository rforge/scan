...
#' @param m The mean depcting the overall distribution of which all cases are a random sample. 
...
estimate_design <- function(data, dvar, pvar, mvar, m = NULL, s = NULL, rtt = NULL, between = TRUE, model = "JW", ...) {
  ...
  if (is.null(m)) 
    m <- mean(sapply(cases, function(x) x$m[1]), na.rm = TRUE)
  if (N > 2 && is.null(s)) 
    s <- sd(sapply(cases, function(x) x$m[1]), na.rm = TRUE)
  ...
  if (is.null(rtt)) 
    rtt <- var(fitted) / (var(fitted) + var(error))
  error_sd <- sqrt(((1 - rtt) / rtt) * s^2)

  VAR <- c("phase", "length", "mt", "rtt", "error", "missing.p", "extreme.p", "extreme.low", 
  ...
  out <- list(
    cases = cases, distribution = "normal", prob = NA
  )
